...
switch ($screen):
	case 'incluir':
		...
		if (isAdmin()== TRUE ):
			...
			if (!isset($_REQUEST['nome']))       $nome   = "erro-envio"; else $nome       = $_REQUEST['nome'];
			if (!isset($_FILES['arquivo']))      $titulo = "";           else $titulo     = $_FILES['arquivo'];
			if (!isset($_FILES['texto1']))       $texto1 = "";           else $texto1     = $_FILES['texto1'];
			
			//Defini o diretorio/galeria que será gravado o arquivo
			$diretorio = $nome."/";
			...
			if (isset($_FILES["arquivo"])){
				...
				for ($i = 0; $i < $numeroCampos; $i++) {
					if ((isset($_POST["cadastrar"])) && (! empty($_FILES["arquivo"]["name"][$i]))){
						...
						$texto1     = $_REQUEST['descrimagem-'.$i];
						$gravadb = new imagens(array(
							'nome'      =>$nome,
							'status'    =>$status,
				...
				echo '<script> location.href="?m=imagens&s=listar" </script>';			
			};			
			?>
			...
			<form class="validaform" method="post" enctype="multipart/form-data" >
				<fieldset>
					<legend>Cadastro - Imagens</legend>
					<ul>
 					
	            		<div >	

							<li><label for="nome">Selecione uma Galeria:</label>
	            			<?php
	            				$lerdb = new galerias();
								...
								while ($resdb= $lerdb->returnData()):
									...
									echo '</br><input type="radio" name="nome" id="nome" value="'.$resdb->nome.'"/> '.$resdb->titulo;
								endwhile;
							?>	
							</li>
		            	</div>
	            		<div >	
							<li><label for="arquivo">Selecione as imagens:</label>
								 <input type="file" name="arquivo[]" /></br>
								 <input type="text" name="descrimagem-0" id="descrimagem-0" size="255" 
									placeholder="Descrição da 1ª imagem escolhida - Tamanho max de 255 caracteres" 
									title="Descrição da 1ª imagem escolhida - Tamanho max de 255 caracteres"  />	
								 <input type="file" name="arquivo[]" /></br>
								 <input type="file" name="arquivo[]" /></br>
								 <input type="file" name="arquivo[]" /></br>
								 <input type="file" name="arquivo[]" /></br>								
						...
						<p>Informe uma breve descrição para cada imagem - na mesma ordem que selecionou os arquivos.</p> 
						<div>
							<li>
								<input type="text" name="descrimagem-0" id="descrimagem-0" size="255" 
								placeholder="Descrição da 1ª imagem escolhida - Tamanho max de 255 caracteres" 
								title="Descrição da 1ª imagem escolhida - Tamanho max de 255 caracteres"  />	
								<input type="text" name="descrimagem--" id="descrimagem-1" size="255" 
								placeholder="Descrição da 2ª imagem escolhida - Tamanho max de 255 caracteres" 
								title="Descrição da 2ª imagem escolhida - Tamanho max de 255 caracteres"   />
								<input type="text" name="descrimagem-2" id="descrimagem-2" size="255" 
								placeholder="Descrição da 3ª imagem escolhida - Tamanho max de 255 caracteres" 
								title="Descrição da 3ª imagem escolhida - Tamanho max de 255 caracteres"   />
								<input type="text" name="descrimagem-3" id="descrimagem-3" size="255" 
								placeholder="Descrição da 4ª imagem escolhida - Tamanho max de 255 caracteres" 
								title="Descrição da 4ª imagem escolhida - Tamanho max de 255 caracteres"   />								
								<input type="text" name="descrimagem-4" id="descrimagem-4" size="255" 
								placeholder="Descrição da 5ª imagem escolhida - Tamanho max de 255 caracteres" 
								title="Descrição da 5ª imagem escolhida - Tamanho max de 255 caracteres"   />								
							
							</li>
						</div>
						
						<p>Todas as inagens receberão o mesmo Status e Relevancia.</p> 
						<p>Para alterar, após o cadastro acesse Listar imagens (Selecione Alterar).</p>
						<div >
						...
						<div >
							<li class="center"><input type="submit" name="cadastrar" value="Salvar Dados" />
							    <input type="button" onclick="location.href='?m=imagens&s=listar'" value="Cancelar" />
							</li>
						</div> 
					</ul>
				</fieldset>	
			...
			$pg = "m=imagens&s=listar";
			$extra_msg = '<a href="?m=imagens&s=listar" onclick="history.back()">Voltar</a>';
			getMSG('cb-909','alerta',$extra_msg);
		endif;		
	break;
	case 'listar':
		echo '<h2>Imagens Cadastradas</h2>';
		...
		<table cellpadding="0" cellspacing="0" border="0" class="display" id="listaImagens">
			<thead>
				<tr>
					<th>Galeria</th><th>Nome Arqv Imagem</th><th>Imagem</th><th>Sts</th><th>Relev</th><th>Ações</th>
				</tr>
			</thead>
			<tbody>
				<?php
				...
				while ($resdb= $lerdb->returnData()):
					...
					printf('<td><img src="'.IMAGESPATH.'%s/min_%s"></td>',$resdb->nome,$resdb->titulo);
					printf('<td>%s</td>',$status);
					printf('<td>%s</td>',$relevancia) ;
					if ($resdb->id != 0):
						printf('<td class="center"><a href="?m=imagens&s=editar&id=%s" title="Editar"><img src="'.BASEURL.'images/edit.png") 
							alt="Editar galeria" /></a></a> <a href="?m=imagens&s=excluir&id=%s" title="Excluir"><img src="'.BASEURL.'images/delete.png") 
	...
	break;	
	case 'editar':
		echo "<h2>Alteração de Imagens</h2>";
		//$session = new session();
		if (isAdmin()== TRUE ):
			...
			if (isset($_POST['editar'])):
				...
				if ($updtdb ->countline==1):
					...
					echo '<script> location.href="?m=imagens&s=listar" </script>';
					// Final  - Rotina de gravar logs						
					//$extra_msg = '<a href="'.ADMURL.'?m=imagens&s=listar">Exibir Cadastros</a>';
			...
			<form class="validaform" method="post" action="">
				<fieldset>
					<legend>Alteração - Imagens</legend>
					<ul>
						<div >
						...
						<hr>
						<p>Imagem no tamanho real:</p>
						<img src="<?php echo IMAGESPATH.$resdb->nome.'/'.$resdb->titulo ?>" />
					</ul>
				</fieldset>
			</form>		
			<?php
	...
	break;
	case 'excluir':
		echo "<h2>Eclusão de Imagens</h2>";
		//$session = new session();
		if (isAdmin()== TRUE ):
			date_default_timezone_set('America/Sao_Paulo');
			$datacad   = date('Y-m-d H:i:s');
			if (isset($_GET['id'])):
				//faz edição do Imagens
				$id = $_GET['id'];
				$lerdb = new imagens();
				$lerdb->pk_value = $id;
				$lerdb->extra_select = "WHERE id=$id";
				...
				if (isset($_POST['exclusao'])):
					...
					if ($deldb->countline==1):
						...
						$caminho = dirname(dirname(dirname(__FILE__)))."/upload/images/".$resdb->nome.'/'.$resdb->titulo;
						$caminhom = dirname(dirname(dirname(__FILE__)))."/upload/images/".$resdb->nome.'/min_'.$resdb->titulo;
						unlink($caminho);
						unlink($caminhom);
			...
			if(is_dir($caminho)):
			   ...
			   echo "<br>Será excluída a imagem: '<strong>".$resdb->titulo. "</strong>' da Galeria '<strong>".$resdb->nome."</strong>':<br />";    
			...
			<form class="userform" method="post" action="">
				<fieldset>
					<legend>Exclusão - Imagens</legend>
					<ul>
						<li><label for="nome">Galeria:</label>
						<?php echo $resdb->nome;?>  			
